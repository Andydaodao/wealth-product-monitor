...
NAV_API_URL = "https://www.bocwm.cn/webApi/cms/productNetWorth/getNetWorthByCode"
NAV_SOURCE_NAME = "中国银行理财产品净值"
HEADERS = {"User-Agent": "Personal-Wealth-Product-Monitor/1.0 (+local personal use)"}
INVALID_NAV_SNAPSHOT_DATE = "2026-09-14"
...
def parse_nav_api(payload: dict, source_url: str) -> tuple[list[dict], int]:
    """Parse the same public JSON data used by the BOCWM product detail page."""
    if not payload.get("result") or not isinstance(payload.get("data"), list):
        return [], 0
    rows = []
    for item in payload["data"]:
        code = str(item.get("productCode") or "").strip().upper()
        nav_date = str(item.get("releaseDate") or "").strip()
        if not code or not re.fullmatch(r"20\d{2}-\d{2}-\d{2}", nav_date):
            continue
        rows.append({
            "product_code": code,
            "nav_date": nav_date,
            "unit_nav": _number(str(item.get("shareNetWorth") or "")),
            "cumulative_nav": _number(str(item.get("cumulativeNetWorth") or "")),
            "ten_thousand_income": _number(str(item.get("eachTenThousandProfit") or "")),
            "seven_day_annualized": _number(str(item.get("sevenDayAnnualization") or "")),
            "source_url": source_url,
        })
    return rows, int(payload.get("total") or len(rows))
...
async def refresh_nav_history(client: httpx.AsyncClient) -> tuple[int, int]:
    with connect() as db:
        products = db.execute(
            "SELECT product_code,source_url FROM products WHERE product_code IS NOT NULL "
    ...
    for product in products:
        ...
        page_no = 1
        total = 1
        while (page_no - 1) * page_size < total:
            response = await client.get(
                NAV_API_URL,
                params={"productCode": code, "pageNo": page_no, "pageSize": page_size},
            )
            response.raise_for_status()
            rows, total = parse_nav_api(response.json(), product["source_url"])
            matching_rows = [row for row in rows if row["product_code"] == code]
            item_count += len(matching_rows)
            new_count += upsert_nav_rows(matching_rows)
            if not rows:
                break
            page_no += 1
...
def seed_public_snapshot() -> None:
    ...
    # 旧预览曾把首页展示时间误当成净值日期，只删除完全匹配的错误快照。
    with connect() as db:
        db.executemany(
            "DELETE FROM nav_history WHERE product_code=? AND nav_date=? "
            "AND unit_nav=? AND cumulative_nav=?",
            [
                (item["code"], INVALID_NAV_SNAPSHOT_DATE, item["nav"], item["nav"])
                for item in PUBLIC_SNAPSHOT
        ...
        try:
            ...
            with connect() as db:
                db.execute(
                    ...
                    "VALUES(?,?,?,?,?,?,?,?)",
                    (scan_id, NAV_SOURCE_NAME, NAV_API_URL, stamp, "正常", 200, item_count, 0),
                )
            results.append({
            ...
            with connect() as db:
                db.execute(
                    ...
                    "VALUES(?,?,?,?,?,?,?,?)",
                    (scan_id, NAV_SOURCE_NAME, NAV_API_URL, stamp, "异常", 0, 0, message),
                )
