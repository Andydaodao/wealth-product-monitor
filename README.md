...
“我的持仓”页面把买入和赎回份额记录保存在当前浏览器的 `localStorage` 中，不上传 GitHub、GitHub Actions 或银行。公开净值由云端定时更新，页面打开后自动按最新净值重新估算持仓价值、最新披露日收益和记录以来净值收益。周末或节假日没有新净值时沿用最近披露数据并显示净值日期。本版未加密，也不跨设备同步；更换浏览器或清除网站数据后记录不会自动恢复。

Windows 通知、ntfy、PDF 附件解析、中国银行动态查询平台、完整变更事件与产品合并、同系列历史对比、24 小时稳定性验证尚未完成。当前电脑的企业代理要求认证，银行实时抓取尚未验证成功；不会绕过代理或银行安全控制。

公开来源：[中银理财产品首页](https://www.bocwm.cn/)、[产品公告](https://www.bocwm.cn/html/1/198/197/index.html)、[产品净值表现](https://www.bocwm.cn/html/1/198/201/index.html)、[纯债7天持有期2号详情](https://www.bocwm.cn/html/1/4/9494.html)。

## 发布到 GitHub Pages

项目保留本地 FastAPI 版本，同时支持生成 GitHub Pages 静态网站。静态版在浏览器中完成搜索和筛选，不能直接修改本地关注列表。GitHub Actions 在北京时间约 09:00 和 21:00 自动扫描；GitHub 调度繁忙时可能稍有延迟。
...
浏览器打开 <http://127.0.0.1:8000>。生成目录 `public/` 不提交，Actions 会在云端重新生成。`data/cloud_state.json` 保存公开产品、净值历史、发现事件和最近的来源状态，供下一次定时扫描比较；本地数据库和关注列表不会上传。
