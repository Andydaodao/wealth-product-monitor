...
Dashboard 会分别展示各数据源的最新状态、最近 4 次扫描的扫描条数与新增条数，以及最近发现的产品。扫描历史从部署本功能后的第一次云端检查开始积累。在 GitHub 网页向 `main` 分支提交代码后，工作流会自动扫描并发布；每天两次的定时扫描和手动触发也继续保留。工作流自己提交的 `data/cloud_state.json` 更新不会再次触发工作流。
